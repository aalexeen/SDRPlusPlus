...
void gen_init_flex(struct gen_params *p, struct gen_state *s)
{
     ...
     * errors==99 is a test-only mode for DROP-01 verification: corrupt exactly
     * ONE content word (codewords[msg_start+2] = the 2nd content word) with 5
     * bit flips — well beyond BCH t=2, so it is guaranteed uncorrectable — while
     * leaving sync/FIW/BIW/vector/address and the 1st content word clean. This
     * makes the decoder flag that single word (bch_err) and emit '?' for its
     * three characters, without dropping the page or the whole message. */
    if (p->p.flex.errors == 99) {
        int bad = msg_start + 2; /* 2nd content word (msg_start=3 -> word 5) */
        /* 5 fixed, distinct bit positions across the 31-bit codeword. */
        codewords[bad] ^= (1u << 2) | (1u << 7) | (1u << 11) | (1u << 16) | (1u << 20);
    } else if (p->p.flex.errors == 98) {
        /* Test-only: corrupt the ADDRESS word (codewords[1]) → uncorrectable →
         * exercises the address-word BCH gate in processPhase (page dropped).
         * The whole page must vanish (no callback), matching multimon. */
        codewords[1] ^= (1u << 2) | (1u << 7) | (1u << 11) | (1u << 16) | (1u << 20);
    } else if (p->p.flex.errors == 97) {
        /* Test-only: corrupt the VECTOR word (codewords[2]) → uncorrectable →
         * exercises the vector-word BCH gate in processPhase (page dropped). */
        codewords[2] ^= (1u << 2) | (1u << 7) | (1u << 11) | (1u << 16) | (1u << 20);
    } else if (p->p.flex.errors > 0) {
        /* Original behaviour: inject errors into the first 10 codewords. */
        for (i = 0; i < FLEX_CODEWORDS_PER_PHASE; i++) {
