...
namespace flex_next_decoder {
    ...
    FlexMessageDecoder::FlexMessageDecoder(std::shared_ptr<FlexGroupHandler> group_handler, int verbosity_level)
        : FlexNextDecoder(verbosity_level), group_handler_(std::move(group_handler)) {
        initializeParsers();
    ...
    MessageParseResult FlexMessageDecoder::parseMessage(const MessageParseInput& input, const DecodingOptions& options) {
        ...
        try {
            ...
            if (options.enable_fragment_assembly && result.success) {
                processFragment(result);
            }
            ...
            if (options.enable_statistics && statistics_enabled_) {
            ...
            if (options.error_callback) {
                options.error_callback(result.error_message);
            }
        }
    ...
    void FlexMessageDecoder::setDecodingOptions(const DecodingOptions& options) {
        options_ = options;
    }

    const DecodingOptions& FlexMessageDecoder::getDecodingOptions() const {
        return options_;
    }
    ...
    bool FlexMessageDecoder::processFragment(MessageParseResult& result) {
        ...
        if (result.fragment_flag == FragmentFlag::Fragment) {
            ...
            if (buffer.fragment_count == 0) {
                buffer.capcode = capcode;
            }
    ...
    void FlexMessageDecoder::clearFragmentBuffers() {
        fragment_buffers_.clear();
    }

    size_t FlexMessageDecoder::getPendingFragmentCount() const {
        return fragment_buffers_.size();
    }

    const MessageStatistics& FlexMessageDecoder::getStatistics() const {
        return statistics_;
    }

    void FlexMessageDecoder::resetStatistics() {
        statistics_.reset();
    }

    void FlexMessageDecoder::setStatisticsEnabled(bool enabled) {
    ...
    void FlexMessageDecoder::processGroupMessage(MessageParseResult& result) {
        if (!group_handler_ || result.group_data.isEmpty()) {
            return;
        }
        ...
        try {
            ...
            if (group_info.isValid()) {
                ...
                if (options_.group_callback) {
                    options_.group_callback(group_info);
                }
            }
        }
        catch (const std::exception& e) {
    ...
    void FlexMessageDecoder::updateStatistics(const MessageParseInput& input, const MessageParseResult& result) {
        ...
        if (input.long_address) {
            statistics_.long_address_messages++;
        }
    }
