...
uint32_t FlexSynchronizer::processSymbol(uint8_t symbol) {
...
uint32_t FlexSynchronizer::checkSyncPattern(uint64_t buffer) const {
    uint32_t marker;
    uint16_t codehigh, codelow;
    ...
    if (countBitDifferences(static_cast<uint32_t>(codehigh), static_cast<uint32_t>(codelow)) >= HAMMING_THRESHOLD) {
        return 0;
    }
    
    // ✅ Valid sync pattern found, return the high code
    return static_cast<uint32_t>(codehigh);
}
...
bool FlexSynchronizer::decodeSyncMode(uint32_t sync_code, SyncInfo& sync_info) {
...
uint32_t FlexSynchronizer::countBitDifferences(uint32_t a, uint32_t b) {
