...
FlexFrameProcessor::FlexFrameProcessor(std::shared_ptr<FlexErrorCorrector> error_corrector,
    ...
    : error_corrector_(std::move(error_corrector))
    , message_decoder_(std::move(message_decoder))
...
FrameProcessingResult FlexFrameProcessor::processFrame(const FlexDataCollector& phase_data_collector,
    ...
    for (char phase_name : active_phases) {
        ...
        switch (phase_name) {
            case 'A': phase_buffer = &phase_data_collector.getPhaseA(); break;
            case 'B': phase_buffer = &phase_data_collector.getPhaseB(); break;
            case 'C': phase_buffer = &phase_data_collector.getPhaseC(); break;
            case 'D': phase_buffer = &phase_data_collector.getPhaseD(); break;
            default:
...
MessageParseResult FlexFrameProcessor::parseMessageContent(const AddressInfoWord& address_info,
    ...
    // Handle group message processing if needed
    if (result.success && !result.group_data.isEmpty() && group_handler_) {
